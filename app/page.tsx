  ...
  const fetchWeather = async (lat: number, lng: number) => {
    try {
      ...
      const relevantHours = data.hourly.time
        ...
        .filter(
          (item: any) =>
            item.time.startsWith(currentDate) &&
            item.hour >= 5 &&
            item.hour <= 17
        );
  ...
  const generateRecommendation = (hourlyData: any[]) => {
        ...
        if (hourDiff <= 1) {
          ...
          if (currentWindow.length >= 2) {
            windows.push({
              ...
              avgTemp: Math.round(
                currentWindow.reduce((sum, h) => sum + h.temperature, 0) /
                  currentWindow.length
              ),
              avgHumidity: Math.round(
                currentWindow.reduce((sum, h) => sum + h.humidity, 0) /
                  currentWindow.length
              ),
              avgWind: Math.round(
                currentWindow.reduce((sum, h) => sum + h.windSpeed, 0) /
                  currentWindow.length
              ),
            });
          }
    ...
    if (windows.length > 0) {
      const bestWindow = windows.reduce((best, current) =>
        current.endHour - current.startHour > best.endHour - best.startHour
          ? current
          : best
      );
      ...
      setRecommendation(
        `Mag laba ka na! Perfect na ang panahon from ${startTime} to ${endTime}! ☀️`
      );
    } else {
      const hasClearWeather = hourlyData.some(
        (item) => item.weatherCode >= 0 && item.weatherCode <= 3
      );

      if (hasClearWeather) {
        setRecommendation(
          "Pwede mag laba, pero hindi ideal ang conditions. Bantayan mo ang damit! ⛅"
        );
      } else {
        setRecommendation(
          "Hindi maganda ang panahon ngayon. Better wait na lang. 🌫️"
        );
      }
    }
  };
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <Card className={`${location ? "bg-blue-50 border-blue-200" : ""}`}>
          <CardHeader className={location ? "pb-2" : ""}>
            ...
            >
              <MapPin
                className={`h-5 w-5 text-blue-600 ${location ? "h-4 w-4" : ""}`}
              />
              Your Location
            </CardTitle>
            {!location && (
              <CardDescription className="text-blue-600">
                We need your location to check the weather
              </CardDescription>
            )}
          </CardHeader>
                    ...
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                      {searchResults.map((result, index) => (
                        <button
                          ...
                          disabled={loading}
                          className="w-full text-left p-3 rounded-lg border border-blue-200 hover:border-blue-400 hover:bg-blue-50 transition-colors disabled:opacity-50"
                        >
          ...
          <Card className="border-blue-200 shadow-lg">
            <CardHeader>
              <CardTitle className="text-blue-900">
                Today's Laundry Forecast
              </CardTitle>
              <CardDescription className="text-blue-700">
                5 AM - 5 PM weather conditions
              </CardDescription>
            </CardHeader>
                ...
                <Card className="bg-blue-50 border-blue-300">
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="text-xs text-blue-800 bg-blue-100 p-3 rounded border border-blue-200">
                      💡 Ideal conditions: 20-30°C, low humidity (&lt;70%), good
                      airflow (&gt;5 km/h). Clothes typically dry in 2-6 hours
